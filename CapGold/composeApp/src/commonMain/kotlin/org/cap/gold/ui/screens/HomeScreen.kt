...
fun HomeScreen(
    ...
    val currentScreen = remember { mutableStateOf<AppScreen>(AppScreen.Products) }
    ...
    val screens = remember(user) {
        AppScreen.items.filter { screen ->
            screen != AppScreen.Users || user.role==0
        }
    }
    ...
    Scaffold(
        bottomBar = {
            BottomNavigationBar(
                screens = screens as List<AppScreen?>,
                currentRoute = currentScreen.value.route,
                onNavigate = { route ->
                    val screen = screens.find { it.route == route } ?: AppScreen.Products
                    if (screen.route != currentScreen.value.route) {
                        currentScreen.value = screen
                        // Create and navigate to the screen with the required parameters
...
private fun BottomNavigationBar(
    ...
    NavigationBar {
        screens.filterNotNull().forEach { screen ->
            NavigationBarItem(
                icon = { 
                    Icon(
